...
/**
 * Service layer for customer operations with Dapr event publishing
 * 
 * Features:
 * - Complete CRUD operations with validation
 * - Event publishing to Azure Service Bus via Dapr for all operations
...
public class CustomerService {
    ...
    private final DaprCustomerEventPublisher eventPublisher;
    private final Tracer tracer;

    public CustomerService(CustomerRepository customerRepository, 
                          DaprCustomerEventPublisher eventPublisher,
                          Tracer tracer) {
    ...
    /**
     * Creates a new customer and publishes a creation event via Dapr
     */
    public CustomerResponse createCustomer(CreateCustomerRequest request) {
        Span span = tracer.spanBuilder("customer.create")
                ...
                .setAttribute("integration.dapr", true)
                .startSpan();
        
        try (Scope scope = span.makeCurrent()) {
            logger.info("Creating customer with email: {} [Dapr Integration]", request.email());
            ...
            logger.info("Customer created successfully: id={}, email={} [Dapr Integration]", savedCustomer.getId(), savedCustomer.getEmail());

            // Publish creation event via Dapr
            try {
                span.addEvent("dapr.event_publish.start");
                eventPublisher.publishCustomerCreated(
                    savedCustomer.getId(), 
                    savedCustomer.getName(), 
                    savedCustomer.getEmail()
                );
                span.addEvent("dapr.event_publish.complete");
            } catch (Exception e) {
                span.recordException(e);
                span.addEvent("dapr.event_publish.failed");
                logger.error("Failed to publish customer creation event via Dapr for id={}, continuing anyway", savedCustomer.getId(), e);
    ...
    /**
     * Updates an existing customer and publishes an update event via Dapr
     */
    public CustomerResponse updateCustomer(Integer id, UpdateCustomerRequest request) {
        ...
        try (Scope scope = span.makeCurrent()) {
            logger.info("Updating customer with id: {} [Dapr Integration]", id);
            ...
            logger.info("Customer updated successfully: id={}, email={} [Dapr Integration]", updatedCustomer.getId(), updatedCustomer.getEmail());

            // Publish update event via Dapr
            try {
                span.addEvent("dapr.event_publish.start");
                ...
                span.recordException(e);
                span.addEvent("dapr.event_publish.failed");
                logger.error("Failed to publish customer update event via Dapr for id={}, continuing anyway", updatedCustomer.getId(), e);
                // Don't fail the operation if event publishing fails
            }
    ...
    /**
     * Deletes a customer and publishes a deletion event via Dapr
     */
    public void deleteCustomer(Integer id) {
        ...
        try (Scope scope = span.makeCurrent()) {
            logger.info("Deleting customer with id: {} [Dapr Integration]", id);
            ...
            logger.info("Customer deleted successfully: id={}, name={} [Dapr Integration]", id, customerName);

            // Publish deletion event via Dapr
            try {
                span.addEvent("dapr.event_publish.start");
                ...
                span.recordException(e);
                span.addEvent("dapr.event_publish.failed");
                logger.error("Failed to publish customer deletion event via Dapr for id={}, continuing anyway", id, e);
                // Don't fail the operation if event publishing fails
            }
        } catch (Exception e) {
            span.recordException(e);
